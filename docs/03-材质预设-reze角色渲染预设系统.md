...
## 六、怎么在界面上实时切换（HUD，现代化停靠面板）

运行（F5）后，所有控制面板收进**一个 HUD 系统**（CanvasLayer，layer 128），分成两块**停靠在屏幕边缘**的面板，**永不直接盖住居中的角色**：
- **左上「播放 / 外观」面板**：动作播放（时间/速度/循环/IK/付与親/表情）+ 描边粗细 + reze 预设（Look 下拉 `ag`/`wuwa`、Grade 下拉 6 套）。
- **右上「材质预设 (reze)」面板**：两个标签页 ——「逐部位编辑」与「部件预设指派」（见第九、十节）。
- **右上角常驻「≡ 面板」按钮（或快捷键 H）**：一键隐藏/显示所有面板，方便纯看画面。每个面板标题栏还有「▾」可单独折叠。
- **文字/控件绝不跑出窗口**：面板高度按视口夹取，内容过多时在面板内滚动；窗口缩放时自动重新夹取位置与尺寸。
- 空白处鼠标穿透到 3D 视口，不影响拖相机。

选完立刻套用到所有材质。代码在 `mmd_importer.gd` 的 `_init_hud()` → `_build_left_dock()` / `_build_right_dock()`，实际调用 `_mp_presets.apply_look(_mats, pack, grade)`。
...
我们这边用**运行时 HUD**实现同一能力（F5 后右侧停靠「材质预设」面板里的**「逐部位编辑」标签页**）：
...
**② HUD（F5 后右侧停靠「材质预设」面板里的**「部件预设指派」标签页**）**：
