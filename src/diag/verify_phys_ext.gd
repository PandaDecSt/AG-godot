...
# Headless probe for the mmd_phys_ext GDExtension.
# Uses runtime (string-based) class lookup so a missing class is reported
# instead of causing a hard parse error.

func _initialize():
	print("=== mmd_phys_ext probe ===")

	var exists = false
	if ClassDB.class_exists("MMDPhysics3D"):
		exists = true
	print("MMDPhysics3D registered: ", exists)

	if not exists:
		print("NOTE: class not found. In --script mode Godot may skip GDExtension")
		print("      loading; this does NOT prove the extension is broken.")
		# List any GDExtension-ish classes for diagnostics
		quit(2)
		return
